...
async def get_xkcd_image(session): # dont wait for the response of API
    rd = randint(0, 1000)
    result = await session.get(f'https://xkcd.com/{rd}/info.0.json') # dont wait for the response
...
async def get_multiple_images(number): 
    async with httpx.AsyncClient() as session: # async client used for async functions
        ...
        result = await asyncio.gather(*tasks, return_exceptions=True) # gather used to collect all
    return result
